...
export async function POST(request) {
  try {
    ...
    // Vérifier le type de fichier (vidéos)
    const allowedTypes = [
      'video/mp4',
      'video/webm',
      'video/ogg',
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json(
        { success: false, error: 'Type de fichier non autorisé. Utilisez MP4, WebM, OGG, MOV ou AVI.' },
        { status: 400 }
      )
    }
    ...
    if (file.size > maxSize) {
      return NextResponse.json(
        { success: false, error: 'Fichier trop volumineux. Taille maximale : 50MB.' },
        { status: 400 }
      )
    }
    ...
    const fileName = `projet_video_${timestamp}_${randomString}.${fileExtension}`
    ...
    try {
      ...
      try {
        ...
        return NextResponse.json({
          ...
          message: 'Vidéo uploadée avec succès (via API REST)'
        })
